...
The Code Card avatar function is designed to assemble and display a custom, personalised image on the Code Card e-paper display. The custom image includes a unique card-owner identicon which is generated on-the-fly via a 3rd party API, and is based on a hash of the card owner's name.

![alt text](images/codecard-avatar-photo-v0.02.png "Code Card Custom Avatar")

The Code Card avatar function is a great way to turn your Code Card into an awesome, personalised name badge - and it's a great reference for building an Oracle function that interracts with a range of OCI services, external services, and the IoT device.
